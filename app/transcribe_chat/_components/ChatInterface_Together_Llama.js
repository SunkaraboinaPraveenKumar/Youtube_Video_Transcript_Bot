...
import { ChatHistoryTranscribe } from "@/config/schema";
...
function ChatInterface({ messages, setMessages }) {
    ...
    const fetchChatHistory = async () => {
        ...
        try {
            ...
            const chatHistory = await db
                .select(ChatHistoryTranscribe)
                .from(ChatHistoryTranscribe)
                .where(eq(ChatHistoryTranscribe.fileId, fileId))
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const fullPrompt = `
                You are a highly knowledgeable, interactive, and user-friendly chatbot designed to provide comprehensive and engaging responses to any question based on context else your own knowledge you can use. Your goals include:

                Relevant Content: "${combinedText}"
                Analyze this context and do following.

...
function WorkSpaceHeader({ messages,setMessages }) {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const response = await db
        ...
        .onConflictDoUpdate({
          target: ChatHistoryTranscribe.fileId,
  ...
  return (
    <div className={`p-4 flex justify-between shadow-md ${pathname !== `/transcribe_chat/${fileId}` ? 'bg-white' : 'bg-gray-400'}`}>
      <Link href={"/dashboard"}>
        <Image src={"/logo.png"} width={50} height={50} alt="logo" />
      </Link>
      <div className="flex gap-2 items-center">
        <Button onClick={onDelete} className='bg-red-500 hover:bg-red-800'>Delete Chat</Button>
